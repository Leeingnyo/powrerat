class Pikachu < ActiveRecord::Base
	def self.damaging
		...
		while true
			...
			if pikachu.remaining_power < 0
				pikachu.cause_of_death = (Site.usage_all (Time.now.to_i.to_s + "000").to_i).to_json
				pikachu.cause_of_death_2 = (Site.active_appliances_all).to_json
				pikachu.save
				pikachu = nil
			end
		end
	...
	def self.info(id)
		...
		return {survival: a.survival, remaining_power: a.remaining_power \
			, max_power: a.max_power, cause_of_deaths: [a.cause_of_death, a.cause_of_death_2], until: all.length}
	end
end
